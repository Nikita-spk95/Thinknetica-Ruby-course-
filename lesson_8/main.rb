...
class Main
  ...
  def trains_list_on_station
    ...
    station.each_with_index do |train, index|
      ...
      if train.type == :passenger
        train.carriages_info do |car|
          puts "  Тип вагона: #{car.type}, Количество мест: #{car.seats}, Количеcтво свободных мест: #{car.free_seats}"
        end
      elsif train.type == :cargo
        train.carriages_info do |car|
          puts "  Тип вагона: #{car.type}, объем: #{car.volume}, Свободное пространство: #{car.free_volume}"
        end
      end
    end
  end
